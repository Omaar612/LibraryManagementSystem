...
type MessageBox(title: string, message: string) as this =
    inherit Window()
    do
        this.Title <- title; this.Width <- 300.0; this.SizeToContent <- SizeToContent.Height
        ...
        s.Children.Add(b); this.Content <- s
...
type AddExistingDialog(titles: string list) as this =
    ...
    do
        this.Title <- "Add Copies"; this.Width <- 300.0; this.SizeToContent <- SizeToContent.Height
        this.WindowStartupLocation <- WindowStartupLocation.CenterOwner
        let s = StackPanel(Margin=Thickness(20.0), Spacing=15.0)
        s.Children.Add(TextBlock(Text="Select Book:")); s.Children.Add(cmb)
        s.Children.Add(TextBlock(Text="Quantity:")); s.Children.Add(txt)
...
type BorrowInfoDialog() as this =
    ...
    let err = TextBlock(Foreground=Brushes.Red, FontWeight=FontWeight.Bold, TextWrapping=TextWrapping.Wrap)
    do
        this.Title <- "Borrow"; this.Width <- 300.0; this.SizeToContent <- SizeToContent.Height
        this.WindowStartupLocation <- WindowStartupLocation.CenterOwner
        let s = StackPanel(Margin=Thickness(20.0), Spacing=15.0)
        s.Children.Add(TextBlock(Text="Name:")); s.Children.Add(tName)
        s.Children.Add(TextBlock(Text="Phone:")); s.Children.Add(tPhone)
        s.Children.Add(err); s.Children.Add(btn)
        this.Content <- s
        btn.Click.Add(fun _ -> 
            err.Text <- ""
...
type ReturnDialog(borrowers: (string * string) list) as this =
    ...
    do
        this.Title <- "Return"; this.Width <- 350.0; this.SizeToContent <- SizeToContent.Height
        this.WindowStartupLocation <- WindowStartupLocation.CenterOwner
        let s = StackPanel(Margin=Thickness(20.0), Spacing=15.0)
        s.Children.Add(TextBlock(Text="Who is returning?")); s.Children.Add(cmb); s.Children.Add(btn)
        this.Content <- s
    ...
    member private this.SetupEvents() =
        ...
        btnAdd.Click.Add(fun _ ->
            ...
            if okQty && okYear && qty > 0 then
                ...
                | Ok _ -> 
                    inputQty.Text <- ""; inputTitle.Text <- ""; inputAuthor.Text <- ""
                    this.RefreshGrid()
